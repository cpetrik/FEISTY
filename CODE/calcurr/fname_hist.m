%%%% File naming system
function [fname,simname,outdir] = fname_hist(param,vers)
frate = param.frate;
...
if (frate==0)
    fname = [outdir,'Hist_',vers];
elseif (param.Jsel~=0.1)
    fname = [outdir,'Hist_',vers,'_', sel,'_fish',tfish,'_Juve',tJ];
elseif (param.MFsel~=param.LPsel)
    fname = [outdir,'Hist_',vers,'_fish_F',tF,'_P',tP,'_D',tD];
else
    fname  = [outdir,'Hist_',vers,'_', sel,'_fish',tfish];  
end
