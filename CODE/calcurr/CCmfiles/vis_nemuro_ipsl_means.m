...
%% 
load([fpath 'feisty_ipsl_gridspec.mat'],'TMO','LAT','LON')
load([fpath 'Data_grid_nemuro_ipsl.mat'],'GRD');

load([fpath 'nemuro_ipsl_means.mat']);
...
dmz_tmean_ipsl = double(mean(D_dZm,1,"omitnan"));
dlz_tmean_ipsl = double(mean(D_dZl,1,"omitnan"));

dmz_hist_mean_ipsl = double(mean(D_dZm,2,"omitnan"));
dlz_hist_mean_ipsl = double(mean(D_dZl,2,"omitnan"));

dmz_ssp_mean_ipsl = double(mean(D_dZm,2,"omitnan"));
dlz_ssp_mean_ipsl = double(mean(D_dZl,2,"omitnan"));
...
figure(10)
subplot(3,1,1)
plot(y,ptemp_tmean_ipsl,'r','Linewidth',1); hold on;
plot(y,btemp_tmean_ipsl,'b','Linewidth',1); hold on;
legend('TP','TB')
xlim([y(1) y(end)])
...
subplot(3,1,2)
plot(y,log10(mz_tmean_ipsl),'color',[0 0.5 0.75],'Linewidth',1); hold on;
plot(y,log10(lz_tmean_ipsl),'color',[0 0 0.75],'Linewidth',1); hold on;
plot(y,log10(det_tmean_ipsl),'color',[0.5 0.5 0.5],'Linewidth',1); hold on;
legend('MZ','LZ','Det')
xlim([y(1) y(end)])
ylabel('log_1_0 Biomass (g m^-^2)')

subplot(3,1,3)
plot(y,(dmz_tmean_ipsl),'color',[0 0.5 0.75],'Linewidth',1); hold on;
plot(y,(dlz_tmean_ipsl),'color',[0 0 0.75],'Linewidth',1); hold on;
legend('dMZ','dLZ')
xlim([y(1) y(end)])
%ylim([-3 2])
xlabel('Time (mo)')
...
stamp('')
print('-dpng',[ppath 'IPSL_mean_forcing_ts.png'])
...
H_MZ(GRD.ID) = mz_hist_mean_ipsl;
H_LZ(GRD.ID) = lz_hist_mean_ipsl;
H_MZloss(GRD.ID)=dmz_hist_mean_ipsl;
H_LZloss(GRD.ID)=dlz_hist_mean_ipsl;
H_Tp(GRD.ID) = ptemp_hist_mean_ipsl;
H_Tb(GRD.ID) = btemp_hist_mean_ipsl;
H_Det(GRD.ID) = det_hist_mean_ipsl;

S_MZ(GRD.ID) = mz_ssp_mean_ipsl;
S_LZ(GRD.ID) = lz_ssp_mean_ipsl;
S_MZloss(GRD.ID)=dmz_ssp_mean_ipsl;
S_LZloss(GRD.ID)=dlz_ssp_mean_ipsl;
S_Tp(GRD.ID) = ptemp_ssp_mean_ipsl;
S_Tb(GRD.ID) = btemp_ssp_mean_ipsl;
S_Det(GRD.ID) = det_ssp_mean_ipsl;
...
print('-dpng',[ppath 'temp_nemuro_ipsl_hist_spp.png'])
...
print('-dpng',[ppath 'zoop_nemuro_ipsl_hist.png'])
...
print('-dpng',[ppath 'zoop_nemuro_ipsl_ssp.png'])
...
print('-dpng',[ppath 'det_nemuro_ipsl_hist_spp.png'])
...
print('-dpng',[ppath 'zoop_log10_nemuro_ipsl_hist.png'])
