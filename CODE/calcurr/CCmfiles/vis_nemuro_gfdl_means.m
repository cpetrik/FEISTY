...
%%
fpath='/Volumes/petrik-lab/Feisty/GCM_Data/NEMURO/GFDLdown/';

% Figures
ppath = '/Users/cpetrik/Petrik Lab Group Dropbox/Colleen Petrik/Princeton/FEISTY/CODE/Figs/PNG/CC/';

%% 
load([fpath 'feisty_gfdl_gridspec.mat'],'TMO','LAT','LON')
load([fpath 'Data_grid_nemuro_gfdl.mat'],'GRD');

load([fpath 'nemuro_gfdl_means.mat']);
...
dmz_tmean_gfdl = double(mean(D_dZm,1,"omitnan"));
dlz_tmean_gfdl = double(mean(D_dZl,1,"omitnan"));

dmz_hist_mean_gfdl = double(mean(D_dZm,2,"omitnan"));
dlz_hist_mean_gfdl = double(mean(D_dZl,2,"omitnan"));

dmz_ssp_mean_gfdl = double(mean(D_dZm,2,"omitnan"));
dlz_ssp_mean_gfdl = double(mean(D_dZl,2,"omitnan"));
...
figure(10)
subplot(3,1,1)
plot(y,ptemp_tmean_gfdl,'r','Linewidth',1); hold on;
plot(y,btemp_tmean_gfdl,'b','Linewidth',1); hold on;
legend('TP','TB')
xlim([y(1) y(end)])
...
subplot(3,1,2)
plot(y,log10(mz_tmean_gfdl),'color',[0 0.5 0.75],'Linewidth',1); hold on;
plot(y,log10(lz_tmean_gfdl),'color',[0 0 0.75],'Linewidth',1); hold on;
plot(y,log10(det_tmean_gfdl),'color',[0.5 0.5 0.5],'Linewidth',1); hold on;
legend('MZ','LZ','Det')
xlim([y(1) y(end)])
ylabel('log_1_0 Biomass (g m^-^2)')

subplot(3,1,3)
plot(y,(dmz_tmean_gfdl),'color',[0 0.5 0.75],'Linewidth',1); hold on;
plot(y,(dlz_tmean_gfdl),'color',[0 0 0.75],'Linewidth',1); hold on;
legend('dMZ','dLZ')
xlim([y(1) y(end)])
%ylim([-3 2])
xlabel('Time (mo)')
...
stamp('')
print('-dpng',[ppath 'GFDL_mean_forcing_ts.png'])
...
H_MZ(GRD.ID) = mz_hist_mean_gfdl;
H_LZ(GRD.ID) = lz_hist_mean_gfdl;
H_MZloss(GRD.ID)=dmz_hist_mean_gfdl;
H_LZloss(GRD.ID)=dlz_hist_mean_gfdl;
H_Tp(GRD.ID) = ptemp_hist_mean_gfdl;
H_Tb(GRD.ID) = btemp_hist_mean_gfdl;
H_Det(GRD.ID) = det_hist_mean_gfdl;

S_MZ(GRD.ID) = mz_ssp_mean_gfdl;
S_LZ(GRD.ID) = lz_ssp_mean_gfdl;
S_MZloss(GRD.ID)=dmz_ssp_mean_gfdl;
S_LZloss(GRD.ID)=dlz_ssp_mean_gfdl;
S_Tp(GRD.ID) = ptemp_ssp_mean_gfdl;
S_Tb(GRD.ID) = btemp_ssp_mean_gfdl;
S_Det(GRD.ID) = det_ssp_mean_gfdl;
...
print('-dpng',[ppath 'temp_nemuro_gfdl_hist_spp.png'])
...
print('-dpng',[ppath 'zoop_nemuro_gfdl_hist.png'])
...
print('-dpng',[ppath 'zoop_nemuro_gfdl_ssp.png'])
...
print('-dpng',[ppath 'det_nemuro_gfdl_hist_spp.png'])
...
print('-dpng',[ppath 'zoop_log10_nemuro_gfdl_hist.png'])
