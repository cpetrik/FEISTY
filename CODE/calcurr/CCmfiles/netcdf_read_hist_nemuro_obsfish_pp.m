...
for z=1:3
    ...
    %% MF
    ncid = netcdf.open([fpath 'Hist_' vers '_' harv '_pp_med_f.nc'],'NC_NOWRITE');
    ...
    % LP
    ncid = netcdf.open([fpath 'Hist_' vers '_' harv '_pp_lrg_p.nc'],'NC_NOWRITE');
    [ndims,nvars,ngatts,unlimdimid] = netcdf.inq(ncid);
    for i = 1:nvars
        varname = netcdf.inqVar(ncid, i-1);
        eval([ varname ' = netcdf.getVar(ncid,i-1);']);
    ...
    % LD
    ncid = netcdf.open([fpath 'Hist_' vers '_' harv '_pp_lrg_d.nc'],'NC_NOWRITE');
    [ndims,nvars,ngatts,unlimdimid] = netcdf.inq(ncid);
    for i = 1:nvars
        varname = netcdf.inqVar(ncid, i-1);
        eval([ varname ' = netcdf.getVar(ncid,i-1);']);
    ...
    %%
    save([fpath 'Means_Hist_' vers '_' harv '_pp_' cfile '.mat'],...
