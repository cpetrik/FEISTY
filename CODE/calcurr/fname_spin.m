%%%% File naming system
function [fname,simname] = fname_spin(param,vers)
frate = param.frate;
...
elseif (param.Jsel~=0.1)
    fname = [outdir,'Spinup_',vers,'_', sel,'_fish',tfish,'_Juve',tJ];
elseif (param.MFsel~=param.LPsel)
    fname = [outdir,'Spinup_',vers,'_fish_F',tF,'_P',tP,'_D',tD];
else
    fname  = [outdir,'Spinup_',vers,'_', sel,'_fish',tfish];  
end
