%%%% File naming system
function [fname,simname,sname] = sub_fname_hist_gfdl_nep(param,odir,exper)
...
elseif (isnan(frate))
    fname = [outdir,'NEP10_Hindcast',exper, '_obsfish'];
    sname = [outdir, 'Last_mo_Spinup',exper,'_obsfish'];
...
elseif (param.MFsel~=param.LPsel)
    fname = [outdir,'NEP10_Hindcast',exper, '_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
    sname = [outdir, 'Last_mo_Spinup',exper,'_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
else
    fname = [outdir,'NEP10_Hindcast',exper, '_', sel,'_fish',tfish(2:end)];
