% Spinup for 150 yrs
% Annual means instead of monthly
% Save last year for initializing runs
...
% Last year
sp_mean=mean(SP.bio(:,end),2);
sf_mean=mean(SF.bio(:,end),2);
sd_mean=mean(SD.bio(:,end),2);
mp_mean=mean(MP.bio(:,end),2);
mf_mean=mean(MF.bio(:,end),2);
md_mean=mean(MD.bio(:,end),2);
lp_mean=mean(LP.bio(:,end),2);
ld_mean=mean(LD.bio(:,end),2);
b_mean=mean(Bent.bio(:,end),2);
...
%% Save means
save([fpath 'Means_' exper '_All_fish03_' cfile '.mat'],'time',...
