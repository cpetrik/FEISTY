...
%%
gpath='/Volumes/petrik-lab/Feisty/GCM_Data/MOM6-NWA12/';

load([gpath 'nwa_raw_ocean_static_gridspec.mat'],'geolon','geolat');
load([gpath 'Data_grid_mom6_nwa12.mat'],'GRD');

[ni,nj]=size(geolon);
geolon_t = double(geolon);
geolat_t = double(geolat);
...
title('Spinup')
stamp(exper)
print('-dpng',[ppath exper 'NWA12_all_sizes.png'])
...
ylabel('log10 Biomass (g m^-^2)')
title(['Spinup'])
print('-dpng',[ppath exper 'NWA12_all_types.png'])
...
stamp(exper)
print('-dpng',[ppath exper 'NWA12_BENT.png'])
...
stamp(exper)
print('-dpng',[ppath exper 'NWA12_All_subplot.png'])
...
stamp(exper)
print('-dpng',[ppath exper 'NWA12_ratios_subplot.png'])


%%
% load([gpath 'nwa_raw_ocean_static_gridspec.mat'],'deptho');
% deptho=double(deptho);
% deptho(deptho>1e18)=nan;
% 
% figure
% axesm ('gortho','MapLatLimit',latlim,'MapLonLimit',lonlim,'frame','on',...
%     'Grid','off','FLineWidth',1)
% surfm(geolat_t,geolon_t,log10(deptho))
% cmocean('deep')
% load coastlines;                     
% %h=patchm(coastlat+0.5,coastlon+0.5,'w','FaceColor',[0.75 0.75 0.75]);
% clim([0 3]);
