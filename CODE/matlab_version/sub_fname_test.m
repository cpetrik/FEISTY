%%%% File naming system
function [fname,simname] = sub_fname_test(frate,test)
...
if (frate==0)
    fname = ['/Volumes/MIP/NC/Matlab_new_size/',simname, '/Climatol_',test,'_pristine'];
elseif (Jsel~=0.1)
    fname = ['/Volumes/MIP/NC/Matlab_new_size/',simname, '/Climatol_',test,'_', sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
elseif (MFsel~=LPsel)
    fname = ['/Volumes/MIP/NC/Matlab_new_size/',simname, '/Climatol_',test,'_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
else
    fname  = ['/Volumes/MIP/NC/Matlab_new_size/',simname, '/Climatol_',test,'_', sel,'_fish',tfish(2:end)];  
end
