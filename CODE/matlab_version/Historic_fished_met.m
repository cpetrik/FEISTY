...
S_Sml_f_met = zeros(NX,DAYS);
S_Sml_p_met = zeros(NX,DAYS);
S_Sml_d_met = zeros(NX,DAYS);
S_Med_f_met = zeros(NX,DAYS);
S_Med_p_met = zeros(NX,DAYS);
S_Med_d_met = zeros(NX,DAYS);
S_Lrg_p_met = zeros(NX,DAYS);
S_Lrg_d_met = zeros(NX,DAYS);
...
%% ! Initialize
init_sim = simname;
load(['/Volumes/MIP/NC/Matlab_new_size/',init_sim '/Last_mo_preindust_' init_sim '.mat']);
BENT.mass = BENT.bio;
...
file_sml_f = [fname,'_met_sml_f.nc'];
file_sml_p = [fname,'_met_sml_p.nc'];
file_sml_d = [fname,'_met_sml_d.nc'];
file_med_f = [fname,'_met_med_f.nc'];
file_med_p = [fname,'_met_med_p.nc'];
file_med_d = [fname,'_met_med_d.nc'];
file_lrg_p = [fname,'_met_lrg_p.nc'];
file_lrg_d = [fname,'_met_lrg_d.nc'];
...
vidmetSF   = netcdf.defVar(ncidSF,'met','double',[xy_dim,time_dim]);
...
vidmetSP   = netcdf.defVar(ncidSP,'met','double',[xy_dim,time_dim]);
...
vidmetSD   = netcdf.defVar(ncidSD,'met','double',[xy_dim,time_dim]);
...
vidmetMF   = netcdf.defVar(ncidMF,'met','double',[xy_dim,time_dim]);
...
vidmetMP   = netcdf.defVar(ncidMP,'met','double',[xy_dim,time_dim]);
...
vidmetMD   = netcdf.defVar(ncidMD,'met','double',[xy_dim,time_dim]);
...
vidmetLP   = netcdf.defVar(ncidLP,'met','double',[xy_dim,time_dim]);
...
vidmetLD   = netcdf.defVar(ncidLD,'met','double',[xy_dim,time_dim]);
...
for YR = 1:YEARS % years
    ...
    ti = num2str(YR+1860);
    load(['/Volumes/MIP/POEM_JLD/esm2m_hist/Data_ESM2Mhist_',ti,'.mat'],'COBALT');
...
%         netcdf.putVar(ncidLD,vidbioLD,[0 MNT-1],[NX 1],mean(S_Lrg_d(:,a(i):b(i)),2));
                ...
                netcdf.putVar(ncidSF,vidmetSF,[0 MNT-1],[NX 1],mean(S_Sml_f_met(:,a(i):b(i)),2));
                netcdf.putVar(ncidSP,vidmetSP,[0 MNT-1],[NX 1],mean(S_Sml_p_met(:,a(i):b(i)),2));
                netcdf.putVar(ncidSD,vidmetSD,[0 MNT-1],[NX 1],mean(S_Sml_d_met(:,a(i):b(i)),2));
                netcdf.putVar(ncidMF,vidmetMF,[0 MNT-1],[NX 1],mean(S_Med_f_met(:,a(i):b(i)),2));
                netcdf.putVar(ncidMP,vidmetMP,[0 MNT-1],[NX 1],mean(S_Med_p_met(:,a(i):b(i)),2));
                netcdf.putVar(ncidMD,vidmetMD,[0 MNT-1],[NX 1],mean(S_Med_d_met(:,a(i):b(i)),2));
                netcdf.putVar(ncidLP,vidmetLP,[0 MNT-1],[NX 1],mean(S_Lrg_p_met(:,a(i):b(i)),2));
                netcdf.putVar(ncidLD,vidmetLD,[0 MNT-1],[NX 1],mean(S_Lrg_d_met(:,a(i):b(i)),2));
