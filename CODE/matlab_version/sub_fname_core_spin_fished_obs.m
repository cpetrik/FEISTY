%%%% File naming system
function [fname,simname] = sub_fname_core_spin_fished_obs()
...
if (~isfolder(['/Volumes/MIP/NC/Matlab_new_size/',simname,'/CORE']))
    mkdir(['/Volumes/MIP/NC/Matlab_new_size/',simname,'/CORE'])
end

%! Setup netcdf path to store to
if (Jsel~=0.1)
    fname = ['/Volumes/MIP/NC/Matlab_new_size/',simname, '/CORE/Core_spin_', sel,'_fish_',tfish,'_Juve',tJ(2:end)];
elseif (MFsel~=LPsel)
    fname = ['/Volumes/MIP/NC/Matlab_new_size/',simname, '/CORE/Core_spin_fish_F',tF,'_P',tP,'_D',tD];
else
    fname  = ['/Volumes/MIP/NC/Matlab_new_size/',simname, '/CORE/Core_spin_', sel,'_fish_',tfish];
end
