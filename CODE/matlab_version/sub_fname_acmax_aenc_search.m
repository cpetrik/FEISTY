%%%% File naming system
function fname = sub_fname_acmax_aenc_search()
...
simname = [coup,'_enc-b',tbenc(2:end),'_m',tmfn,'-b',tbfn(2:end),'-k',tkfn(2:end),'_c-b',tbcmx(2:end),'_D',td(2:end),'_J',tj(2:end),'_A',ta(2:end),'_Sm',tsm(2:end),'_nmort',tmort,'_BE',tbe(2:end),'_noCC_RE',tre(2:end)];
ptext = ['enc',tefn,'_cmax',tcfn];
...
if (frate==0)
    fname = ['/Volumes/MIP/NC/Matlab_new_size/param_ensemble/',simname, '/Climatol_pristine_' ptext];
else
    fname  = ['/Volumes/MIP/NC/Matlab_new_size/param_ensemble/',simname, '/Climatol_', sel,'_fish',tfish(2:end),'_' ptext];
end
