...
[rPD,pPD]=corr(sFracPD(keep),plme_rPDcatch(keep));
...
FPD=10^(median(sFracPD(keep)-plme_rPDcatch(keep)));
...
writetable(Fstat,[dpath 'US_LME_DvD_SAU_stats_' cfile '.csv'],'Delimiter',',','WriteRowNames',true)
save([dpath 'US_LME_DvD_SAU_stats_' cfile '.mat'],'fish_stat')
...
cmocean('balance')
colorbar('Position',[0.49 0.6 0.025 0.34])
load coast;                     %decent looking coastlines
h=patchm(lat+0.5,long+0.5,'w','FaceColor',[0.75 0.75 0.75]);
caxis([-1 1]);
set(gcf,'renderer','painters')
text(-1.3,1.9,'B','FontSize',14)
%title('FEISTY  - vanD difference')

%SAU corr
subplot('Position',[0.075 0.1 0.38 0.4])
plot(x,x,'--k');hold on;
scatter(sFracPD(keep),plme_rPDcatch(keep),50,'k','filled'); hold on;
...
plot(x,x,'--k');hold on;
scatter(FracLP(did),plme_rPDcatch(did),50,'k','filled'); hold on;
...
print('-dpng',[ppath 'Clim_' harv '_US_LME_fracPD_catch_SAUP_DvD_comp_subplot.png'])
...
print('-dpng',[ppath 'Clim_' harv '_US_LME_fracPD_catch_SAUP_comp_subplot.png'])
