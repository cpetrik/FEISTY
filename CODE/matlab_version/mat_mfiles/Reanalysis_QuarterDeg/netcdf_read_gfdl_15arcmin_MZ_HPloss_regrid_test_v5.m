...
% Only remapped MZ, but does it correctly - 19910101 
% (but 1971 run on kelpfish 8/6/24 did not?)
% 1972 run on tunafish 8/7/24 correct
...
%% zmeso regular grid
ncdisp([fpath '19720101.ocean_cobalt_MZflux_int_FishMIP_CP_remapped.nc'])
...
%% 
ncid = netcdf.open([fpath '19720101.ocean_cobalt_MZflux_int_FishMIP_CP_remapped.nc'],'NC_NOWRITE');
...
%%
test = double(squeeze(hploss_nmdz_100(:,:,6)));
...
tsplit1 = test(1:720,:);
tsplit2 = test(721:end,:);
tflip1 = fliplr(tsplit1);
tflip2 = fliplr(tsplit2);
tcomb = [tflip2;tflip1]; %This looks aligned with ISIMIP depth

close all
figure
pcolor(tflip1); shading flat

figure
pcolor(tflip2); shading flat

figure
pcolor(tcomb); shading flat
