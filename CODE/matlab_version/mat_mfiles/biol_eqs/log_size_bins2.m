...
%% Solve for slope
sM = 2*BMZ / (2*mbin_mids(1)*Hm - Hm^2);
sL = 2*BLZ / (2*lbin_mids(1)*Hl - Hl^2);
...
aM1 = sM * mbin_mids(1);
aL1 = sL * lbin_mids(1);
BM(1) = aM1*hm - 0.5*hm^2 * sM;
BL(1) = aL1*hl - 0.5*hl^2 * sL;

for i=2:length(lbin_mids)
    BM(i) = aM1*hm - 0.5*hm^2 * sM - (i-1)*hm^2 * sM;
    BL(i) = aL1*hl - 0.5*hl^2 * sL - (i-1)*hl^2 * sL;
end

% ALSO WORKS BUT I GET NEG VALUES
