function [TEeffM,TEeff_ATL,TEeff_LTLd,TEeff_HTLd] = ...
    fore_fished_effTEs_Det_Zprod_ensem(BE,mnpp,mdet,mmz_prod,mlz_prod,...
...
% Use BE*det
% Use ZoopProd instead of ZoopHPloss
...
mnpp = mnpp(ID);
mdet = mdet(ID);
mmz_prod = mmz_prod(ID);
mlz_prod = mlz_prod(ID);
...
%% save
save([fname '_TEeffDetZooProd_' simname '.mat'],'TEeffM',...
    'AllM','AllL','mmz_prod','mlz_prod','mnpp','mdet',...
