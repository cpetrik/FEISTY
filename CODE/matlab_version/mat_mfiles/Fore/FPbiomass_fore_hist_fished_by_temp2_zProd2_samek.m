...
%%
spred = nan*ones(9,4,length(params));
for j = 1:length(params)
    ...
    % remove high values of z prod b/c only in warm
    lz10 = find(vZ10<=1.2);
    lz20 = find(vZ20<=1.2);
    
    mF10 = fitlm(vZ10(lz10),vF10(lz10),'linear');
    mP10 = fitlm(vZ10(lz10),vP10(lz10),'linear');
    mF20 = fitlm(vZ20(lz20),vF20(lz20),'linear');
    mP20 = fitlm(vZ20(lz20),vP20(lz20),'linear');
    ...
    ylabel('fish biomass (g m^-^2)')
    axis([0 2 0 20])
    %print('-dpng',[pp 'Hist_Fore_',harv,'_FP_zProd2_temp2_scatter_',simname,'.png'])
    
end

save([nfile 'FP_zProd2_temp2_hist_fore_fits_ens_samek.mat'],'spred');
