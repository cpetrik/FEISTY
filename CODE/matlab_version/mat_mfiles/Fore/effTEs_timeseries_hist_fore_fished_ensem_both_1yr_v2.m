...
%% Effective TEs of same k
% With BE*det instead of Bent
% With Zprod instead of Zloss

%TEeff_ATL = production_L/NPP
sTEe_ATL = sLprod1 ./ npp;
sTEe_ATL(sTEe_ATL==-Inf) = NaN;
sTEe_ATL(sTEe_ATL==Inf) = NaN;
sTEe_ATL(sTEe_ATL<0) = NaN;
sTEe_ATL(sTEe_ATL>1) = NaN;

%TEeff_HTL = production_L/(production_benthic_invert+mesozoo_prod)
sTEe_HTL = sLprod1 ./ (BE*det + mz_prod + lz_prod); 
sTEe_HTL(sTEe_HTL<0) = NaN;
sTEe_HTL(sTEe_HTL>1) = NaN;

%Calc TE from effTEs
sTEATL = real(sTEe_ATL.^(1/4));         
sTEHTL = real(sTEe_HTL.^(1/3));

% Means
sTEeff_HTL = squeeze(nanmean(sTEe_HTL));
sTEeff_ATL = squeeze(nanmean(sTEe_ATL));
sTE_HTL = squeeze(nanmean(sTEHTL));
sTE_ATL = squeeze(nanmean(sTEATL));
...
sSa=[msATL+ssATL; flipud(msATL-ssATL)]; 
sSh=[msHTL+ssHTL; flipud(msHTL-ssHTL)];
...
xlim([1951 2095])
xlabel('Year')
ylabel('Change in TE relative to 1951')
text(1951,0.8,'A')
...
xlabel('Year')
text(1951,0.8,'B')
print('-dpng',[ppath 'Hist_Fore_',harv,'_TE_Det_Zprod_d1951_cone_1std_ensem_both_color_v2.png'])
...
plot(y,100*msATL,'k','Linewidth',2); hold on;
fill(X,100*sSh,'c','FaceAlpha',0.25,'EdgeAlpha',0.25); hold on; %plot filled area
fill(X,100*sSa,'k','FaceAlpha',0.25,'EdgeAlpha',0.25); hold on; 
plot(xyr,y0,'--k'); hold on;
legend('LTL','HTL','ATL')
legend('location','southwest')
%ylim([-3.25 0.5])
xlim([1951 2095])
...
%% test diff TE calc
%Calc TE from effTEs
oTEATL = real(TEeff_ATL.^(1/3.6667));         
oTEHTL = real(TEeff_HTL.^(1/2.6667));

% Means
oTE_HTL = nanmean(oTEHTL);
oTE_ATL = nanmean(oTEATL);

%Calc TE from effTEs
sTEATL = real(sTEe_ATL.^(1/3.6667));         
sTEHTL = real(sTEe_HTL.^(1/2.6667));

% Means
sTE_HTL = squeeze(nanmean(sTEHTL));
sTE_ATL = squeeze(nanmean(sTEATL));
msHTL = mean(sTE_HTL,2);
msATL = mean(sTE_ATL,2);

% DIfference from 1951
doHTL = oTE_HTL - oTE_HTL(:,yid);
doATL = oTE_ATL - oTE_ATL(:,yid);
dsHTL = msHTL - msHTL(yid,:);
dsATL = msATL - msATL(yid,:);

%%
figure(2)
subplot(2,2,1)
plot(y,100*dLTL,'color',[0.5 0 1],'Linewidth',2); hold on;
plot(y,100*doHTL,'color',[0 0.5 0.75],'Linewidth',2); hold on;
plot(y,100*doATL,'k','Linewidth',2); hold on;
ylim([-3.25 0.5])
xlim([1951 2095])
xlabel('Year')
ylabel('Change in TE relative to 1951')
...
subplot(2,2,2)
plot(y,100*dLTL,'color',[0.5 0 1],'Linewidth',2); hold on;
plot(y,100*dsHTL,'color',[0 0.5 0.75],'Linewidth',2); hold on;
plot(y,100*dsATL,'k','Linewidth',2); hold on;
legend('LTL','HTL','ATL')
legend('location','southwest')
ylim([-3.25 0.5])
xlim([1951 2095])
xlabel('Year')
text(1951,0.8,'B')
%print('-dpng',[ppath 'Hist_Fore_',harv,'_TEtest_Det_Zprod_d1951_cone_1std_ensem_both_color_v2.png'])
