...
%% var in prod moving means
hyr = (y>=1951 & y<2000);
fyr = (y>=2051 & y<2100);

vt(1,1) = mean(var(mmdF(:,hyr),0,2)); %0.42          All *1e-6
vt(1,2) = mean(var(mmdF(:,fyr),0,2)); %0.16 F decr
vt(2,1) = mean(var(mmdP(:,hyr),0,2)); %0.12
vt(2,2) = mean(var(mmdP(:,fyr),0,2)); %0.09 P decr
vt(3,1) = mean(var(mmdD(:,hyr),0,2)); %0.0011
vt(3,2) = mean(var(mmdD(:,fyr),0,2))  %0.0031 D incr
