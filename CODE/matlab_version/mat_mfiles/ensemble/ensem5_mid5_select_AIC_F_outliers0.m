...
% Remove all F outliers from analysis --> change to zero so same # obs
...
[oboth2,oid]=intersect(keep,over);
...
writetable(cT,[dp 'LHS_param5_mid5_AIC_Foutlier0.csv'])
...
writetable(bT,[dp 'LHS_param5_mid5_AIC_builtin_Foutlier0.csv'])
...
pT = array2table(pset,'VariableNames',{'ParamSet','Lambda','bMet','bEnc',...
    'aMet','aEnc','AIC'});
writetable(pT,[dp 'LHS_param5_mid5_bestAIC_params_Foutlier0.csv'])

id1 = pid;

% 137 PARAMETER SETS??? TOO MANY!
