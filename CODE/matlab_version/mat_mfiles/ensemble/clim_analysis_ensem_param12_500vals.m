...
for j = 1:500
    ...
    fname = sub_fname_ensemble12();
    sim{j} = fname;
end

%%
SF = NaN*ones(45124,length(fx2));
SP = SF;
SD = SF;
MF = SF;
MP = SF;
...
LPc = SF;
LDc = SF;
lme_Fmcatch = NaN*ones(66,length(fx2));
lme_Pmcatch = NaN*ones(66,length(fx2));
lme_Dmcatch = NaN*ones(66,length(fx2));
lme_AllF = NaN*ones(66,length(fx2));
lme_AllP = NaN*ones(66,length(fx2));
r_all = NaN*ones(5,length(fx2));
ss_all = NaN*ones(5,length(fx2));
rmse_all = NaN*ones(5,length(fx2));
mis_all = NaN*ones(length(fx2),45,5);
...
%%
save([dp 'Climatol_ensemble_param12_LHS500.mat'],'SF','SP','SD',...
