...
sim = cell(length(fx_all),1);
for j = 1:length(fx_all)
...
%%
SF = NaN*ones(45124,length(fx_all));
SP = SF;
SD = SF;
MF = SF;
MP = SF;
...
LPc = SF;
LDc = SF;
lme_Fmcatch = NaN*ones(66,length(fx_all));
lme_Pmcatch = NaN*ones(66,length(fx_all));
lme_Dmcatch = NaN*ones(66,length(fx_all));
lme_AllF = NaN*ones(66,length(fx_all));
lme_AllP = NaN*ones(66,length(fx_all));
r_all = NaN*ones(5,length(fx_all));
ss_all = NaN*ones(5,length(fx_all));
rmse_all = NaN*ones(5,length(fx_all));
mis_all = NaN*ones(length(fx_all),45,5);

%%
for M=1:length(fx_all)
    sfile = sim{M};
    sname = sfile(140:end);
    load([sfile '.mat']);
...
%%
save([dp 'Climatol_ensemble_param5_mid5.mat'],'SF','SP','SD',...
    'MF','MP','MD','BI','LP','LD','MFc','MPc','MDc','LPc','LDc',...
    'lme_Fmcatch','lme_Pmcatch','lme_Dmcatch','lme_AllF','lme_AllP',...
    'r_all','rmse_all','ss_all','mis_all','sim','fx_all')
