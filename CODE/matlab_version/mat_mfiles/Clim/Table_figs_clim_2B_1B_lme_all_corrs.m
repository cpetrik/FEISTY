% All correlations between Clim w/ 1 benthos vs. 2 benthos
clear all
close all
...
load([gpath 'LME_clim_temp_zoop_det.mat'],'lme_ptemp');
...
fpath1 = [dp cfile1 '/Climatology/'];
load([fpath1 'LME_clim_fished_',harv,'_' cfile1 '.mat'],'lme_mcatch',...
    'lme_mbio','lme_area');
load([fpath1 'TEeff_Climatol_All_fish03_' cfile1 '.mat'],'lme_te');
...
load([fpath2 'LME_clim_fished_',harv,'_' cfile2 '.mat']);
load([fpath2 'TEeffDet_Climatol_All_fish03_' cfile2 '.mat'],'lme_te');
...
Fstat = array2table(fish_stat,'VariableNames',{'r','RMSE','Bias','p'},...
    ...
    'Frac Large-Med','TEeffATL','TEeffHTL','TEeffLTL'});
writetable(Fstat,[fpath2 'LME_clim_2B_1B_stats_' cfile2 '.csv'],'Delimiter',',',...
    'WriteRowNames',true)
save([fpath2 'LME_clim_2B_1B_stats_' cfile2 '.mat'],'fish_stat')
...
text(-2.75,0.3,['r = ' sprintf('%2.2f',rF) ' (p = ' sprintf('%2.2f',pF) ')'])
text(-2.75,0.0,['RMSE = ' sprintf('%2.2f',rmseF)])
axis([-3 0.5 -3 0.5])
xlabel('2B')
ylabel('1B')
title('Forage Fishes')
...
cmocean('thermal');
text(-4.8,0.7,['r = ' sprintf('%2.2f',rP) ' (p = ' sprintf('%2.2f',pP) ')'])
text(-4.8,0.2,['RMSE = ' sprintf('%2.2f',rmseP)])
axis([-5 1 -5 1])
xlabel('2B')
ylabel('1B')
...
cmocean('thermal');
text(-1.75,0.8,['r = ' sprintf('%2.2f',rall) ' (p = ' sprintf('%2.2f',pall) ')'])
text(-1.75,0.5,['RMSE = ' sprintf('%2.2f',rmse)])
axis([-2 1 -2 1])
xlabel('2B')
ylabel('1B')
title('All fishes')
stamp([harv '_' cfile2])
print('-dpng',[ppath 'Clim_2B_1B_',harv,'_comp_types_temp.png'])
...
cmocean('thermal');
colorbar('Position',[0.375 0.5 0.3 0.025],'orientation','horizontal')
text(0,1.0,['r = ' sprintf('%2.2f',rPF) ' (p = ' sprintf('%2.2f',pPF) ')'])
text(0,0.9,['RMSE = ' sprintf('%2.2f',rmsePF)])
...
cmocean('thermal');
text(0,1,['r = ' sprintf('%2.2f',rPD) ' (p = ' sprintf('%2.2f',pPD) ')'])
text(0,0.9,['RMSE = ' sprintf('%2.2f',rmsePD)])
axis([-0.1 1.1 -0.1 1.1])
xlabel('2B')
ylabel('1B')
title('P / (P+D)')
...
cmocean('thermal');
text(0,1,['r = ' sprintf('%2.2f',rLM) ' (p = ' sprintf('%2.2f',pLM) ')'])
text(0,0.9,['RMSE = ' sprintf('%2.2f',rmseLM)])
axis([-0.1 1.1 -0.1 1.1])
xlabel('2B')
ylabel('1B')
title('L / (L+M)')
...
cmocean('thermal');
text(-3.75,1.6,['r = ' sprintf('%2.2f',rB) ' (p = ' sprintf('%2.2f',pB) ')'])
text(-3.75,1.1,['RMSE = ' sprintf('%2.2f',rmseB)])
axis([-4 2 -4 2])
xlabel('2B')
ylabel('1B')
title('Benthos')
stamp([harv '_' cfile2])
print('-dpng',[ppath 'Clim_2B_1B_',harv,'_comp_fracs_temp.png'])
...
cmocean('thermal');
colorbar('Position',[0.375 0.5 0.3 0.025],'orientation','horizontal')
text(-4.3,-2.15,['r = ' sprintf('%2.2f',rL) ' (p = ' sprintf('%2.2f',pL) ')'])
text(-4.3,-2.45,['RMSE = ' sprintf('%2.2f',rmseL)])
axis([-4.5 -2 -4.5 -2])
xlabel('2B')
ylabel('1B')
...
cmocean('thermal');
text(-3.25,-1.15,['r = ' sprintf('%2.2f',rHTL) ' (p = ' sprintf('%2.2f',pHTL) ')'])
text(-3.25,-1.45,['RMSE = ' sprintf('%2.2f',rmseHTL)])
axis([-3.5 -1 -3.5 -1])
xlabel('2B')
ylabel('1B')
...
cmocean('thermal');
text(-1.9,-0.65,['r = ' sprintf('%2.2f',rLTL) ' (p = ' sprintf('%2.2f',pLTL) ')'])
text(-1.9,-0.8,['RMSE = ' sprintf('%2.2f',rmseLTL)])
axis([-2 -0.5 -2 -0.5])
xlabel('2B')
ylabel('1B')
...
stamp([harv '_' cfile2])
print('-dpng',[ppath 'Clim_2B_1B_',harv,'_comp_TEeffs_temp.png'])
