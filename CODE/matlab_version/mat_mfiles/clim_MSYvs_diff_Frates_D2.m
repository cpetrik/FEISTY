...
load([dp 'Climatol_diff_Frates_D2.mat']);
...
xlabel('F')
title('All')
print('-dpng',[ppath 'Climatol_global_yield_diff_Frates_D2.png'])
...
print('-dpng',[ppath 'Climatol_LME_yield_diff_Frates_D2_All.png'])
...
print('-dpng',[ppath 'Climatol_LME_yield_diff_Frates_D2_F.png'])
...
print('-dpng',[ppath 'Climatol_LME_yield_diff_Frates_D2_P.png'])
...
print('-dpng',[ppath 'Climatol_LME_yield_diff_Frates_D2_D.png'])
...
xlabel('F')
title('All')
print('-dpng',[ppath 'Climatol_yield_diff_Frates_D2_SAUPcomp_rcorr.png'])
...
xlabel('F')
title('All')
print('-dpng',[ppath 'Climatol_yield_diff_Frates_D2_SAUPcomp_rmse.png'])
...
print('-dpng',[ppath 'Climatol_LME_select_yield_diff_Frates_D2_F.png'])
...
for m = 1:nrows
    for n = 1:ncols
        ...
        L = lmes(i);
        subplot('position',pos(m,:,n))
        plot(Fish,(lme_Pmcatch(L,:))*1e-12,'.-b','MarkerSize',10); hold on;
        xlim([0 2])
        title(num2str(L))
        set(gca,'XTickLabel',[])
...
print('-dpng',[ppath 'Climatol_LME_select_yield_diff_Frates_D2_P.png'])
...
for i = 1:length(lmes)
        L = lmes(i);
        plot(Fish,(lme_Pmcatch(L,:))*1e-12,'.-b',...
            'MarkerSize',20,'LineWidth',2); 
        xlim([0 2])
        set(gca,'YTickLabel',[])
        title(num2str(L))
        ylabel('P Catch')
        xlabel('Fishing rate on D (yr^-^1)')
        print('-dpng',[ppath 'LMEs/Climatol_LME',num2str(L),'_yield_diff_Frates_D2_P.png'])
end
...
for i = 1:length(lmes)
        ...
        ylabel('F Catch')
        xlabel('Fishing rate on D (yr^-^1)')
        print('-dpng',[ppath 'LMEs/Climatol_LME',num2str(L),'_yield_diff_Frates_D2_F.png'])
end
