...
ID = grid(:,1);
load([cpath 'LME_hist9095_temp_zoop_det.mat'],'lme_ptemp');
...
writetable(T1,[spath 'LME_TEeff_hist_Mauread_comp_' cfile '.csv'],'Delimiter',',')
save([dpath 'LME_TEeff_hist_Mauread_comp_' cfile '.mat'],'tab','keep',...
    'mECI','pECI')
...
%on grid
tlme = lme_mask_esm2m';
mECI_grid = NaN*ones(ni,nj);
pECI_grid = NaN*ones(ni,nj);
for L=1:length(keep)
    lme=keep(L);
    lid = find(tlme==lme);
...
caxis([-2.8 -1.4]);
colorbar('Position',[0.025 0.5575 0.45 0.035],'orientation','horizontal')                   
set(gcf,'renderer','painters')
title('Historic log_1_0 TEeff HTL')
text(-2.75,1.25,'A')
...
h=patchm(lat+0.5,long+0.5,'w','FaceColor',[0.75 0.75 0.75]);
caxis([-2.8 -1.4]);
colorbar('Position',[0.525 0.5575 0.45 0.035],'orientation','horizontal')                   
set(gcf,'renderer','painters')
...
%Corr
subplot('Position',[0.58 0.13 0.34 0.33])
plot(x,x,'--k');hold on;
...
text(-2.9,-1.15,'D')
stamp(cfile)
print('-dpng',[ppath 'Hist_' harv '_LME_TEeff_Maureaud_comp_subplot_pval.png'])
