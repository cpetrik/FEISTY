...
% g/m2 --> total g
Amf_myield = Zmf .* AREA_OCN * 365; %mean fish catch per yr
Amp_myield = Zmp .* AREA_OCN * 365;
Amd_myield = Zmd .* AREA_OCN * 365;
Alp_myield = Zlp .* AREA_OCN * 365;
Ald_myield= Zld .* AREA_OCN * 365;
...
lme_myield = NaN*ones(66,5);
lme_mcatch = NaN*ones(66,9);
...
for L=1:66
    lid = find(tlme==L);
    %total yield g
    lme_myield(L,1) = nansum(Amf_myield(lid));
    lme_myield(L,2) = nansum(Amp_myield(lid));
    lme_myield(L,3) = nansum(Amd_myield(lid));
    lme_myield(L,4) = nansum(Alp_myield(lid));
    lme_myield(L,5) = nansum(Ald_myield(lid));
    %mean catch g
    lme_mcatch(L,1) = nanmean(Amf_mean(lid));
    lme_mcatch(L,2) = nanmean(Amp_mean(lid));
    lme_mcatch(L,3) = nanmean(Amd_mean(lid));
    lme_mcatch(L,4) = nanmean(Alp_mean(lid));
    lme_mcatch(L,5) = nanmean(Ald_mean(lid));
    %total catch g
...
hlme_myield = nansum(lme_myield,2) * 1e-6;

%%
save([dpath 'LME_hist_',harv,'_' cfile '.mat'],...
    'lme_myield','lme_mcatch','lme_tcatch','lme_area');
...
for L=1:66
    ...
    clme_mf(lid) = lme_myield(L,1);
    clme_mp(lid) = lme_myield(L,2);
    clme_md(lid) = lme_myield(L,3);
    clme_lp(lid) = lme_myield(L,4);
    clme_ld(lid) = lme_myield(L,5);
    
    lme_sf(lid) = lme_mcatch(L,1);
    lme_sp(lid) = lme_mcatch(L,2);
    lme_sd(lid) = lme_mcatch(L,3);
    lme_mf(lid) = lme_mcatch(L,4);
    lme_mp(lid) = lme_mcatch(L,5);
    lme_md(lid) = lme_mcatch(L,6);
    lme_lp(lid) = lme_mcatch(L,7);
    lme_ld(lid) = lme_mcatch(L,8);
    lme_b(lid) = lme_mcatch(L,9);
end
...
title('Historic 1951-2000 LME mean log10 total annual catch (MT) All Fishes')
stamp(cfile)
...
set(gcf,'renderer','painters')
title('Historic 1951-2000 LME mean log10 total annual catch (MT) Forage Fishes')
stamp(cfile)
...
ylim(hcb,[3.5 7.5])                   %Set color axis if needed
set(gcf,'renderer','painters')
title('Historic 1951-2000 LME mean log10 total annual catch (MT) Pelagic Fishes')
stamp(cfile)
...
ylim(hcb,[3.5 7.5])                   %Set color axis if needed
set(gcf,'renderer','painters')
title('Historic 1951-2000 LME mean log10 total annual catch (MT) Demersal Fishes')
stamp(cfile)
...
ylim(hcb,[3.5 7.5])                   %Set color axis if needed
set(gcf,'renderer','painters')
title('Historic 1951-2000 LME mean log10 total annual catch (MT) Medium Fishes')
stamp(cfile)
...
ylim(hcb,[3.5 7.5])                   %Set color axis if needed
set(gcf,'renderer','painters')
title('Historic 1951-2000 LME mean log10 total annual catch (MT) Large Fishes')
stamp(cfile)
