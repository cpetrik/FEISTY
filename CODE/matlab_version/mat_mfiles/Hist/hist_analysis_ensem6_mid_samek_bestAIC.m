...
Hlme_tsc_mf = NaN*ones(nparam,145);
...
%         mf_prod,mp_prod,md_prod,lp_prod,ld_prod,fname,simname);
    ...
    %% Yield
    netcdf_read_hist_fished_yield_ens(fname,simname)
    
    load([fname '_Means_' simname '.mat'],...
        'mf_tsyc','mp_tsyc','md_tsyc','lp_tsyc','ld_tsyc');
    
    % Time series of catch (g per km2 per year)
    Hlme_tsc_mf(j,:) = mf_tsyc; 
    Hlme_tsc_mp(j,:) = mp_tsyc;
    Hlme_tsc_md(j,:) = md_tsyc;
    Hlme_tsc_lp(j,:) = lp_tsyc;
    Hlme_tsc_ld(j,:) = ld_tsyc;
...
save([nfile 'Historic_All_fish03_ensem6_mid_temp3_bestAIC_multFup_multPneg.mat'],...
