...
%%
ncdisp([fpath 'ocean.100101-102012.sst.nc'])
% sst
sst_long_name     = 'Potential temperature';
sst_units         = 'degrees K';
...
for q = 1:length(s20)
    ...
    ncid = netcdf.open([fpath 'ocean.',tspan,'.sst.nc'],'NC_NOWRITE');
    [ndims,nvars,ngatts,unlimdimid] = netcdf.inq(ncid);
    for i = 1:nvars
        varname = netcdf.inqVar(ncid, i-1);
        eval([ varname ' = netcdf.getVar(ncid,i-1);']);
