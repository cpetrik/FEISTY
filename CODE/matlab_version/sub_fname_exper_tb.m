%%%% File naming system
function [fname,simname] = sub_fname_exper_tb(frate)
...
if (frate==0)
    fname = ['/Volumes/FEISTY/NC/Matlab_new_size/',simname, '/Biome_exper_TB_pristine'];
elseif (Jsel~=0.1)
    fname = ['/Volumes/FEISTY/NC/Matlab_new_size/',simname, '/Biome_exper_TB_', sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
elseif (MFsel~=LPsel)
    fname = ['/Volumes/FEISTY/NC/Matlab_new_size/',simname, '/Biome_exper_TB_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
else
    fname  = ['/Volumes/FEISTY/NC/Matlab_new_size/',simname, '/Biome_exper_TB_', sel,'_fish',tfish(2:end)];  
end
