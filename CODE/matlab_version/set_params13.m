...
function set_params13(X)
    ...
    amet = X(3);       % coeff on met
    h    = X(4);         % coeff on Cmax
    gam  = X(5);       % coeff on search area
    kc = X(6);     % coeff on cmax T-dep fn 
    ke = X(7);     % coeff on enc T-dep fn 
    kt = X(8);    % coeff on met T-dep fn (orig 0.063)
    bpow = X(9);   % power on metab fn (orig 0.25)
    benc = X(10);    % power on enc fn (orig 0.20)
    bcmx = X(11);    % power on cmax fn (orig 0.25)
    ...
    A = X(13);    %Adult predation reduction %*****
