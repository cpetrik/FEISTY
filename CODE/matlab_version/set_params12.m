...
function set_params12(X)
    ...
    kt = X(7);    % coeff on met T-dep fn (orig 0.063)
    bpow = X(8);   % power on metab fn (orig 0.25)
    benc = X(9);    % power on enc fn (orig 0.20)
    bcmx = X(10);    % power on cmax fn (orig 0.25)
    ...
    A = X(12);    %Adult predation reduction %*****
