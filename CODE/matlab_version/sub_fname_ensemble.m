%%%% File naming system
function fname = sub_fname_ensemble()
...
simname = [coup,'_D',td(2:end),'_Sm',tsm(2:end),'_nmort',tmort,'_noCC_RE',tre(2:end)];
ptext = ['Lam',tlam(2:end),'_enc',tefn,'-b',tbenc(2:end),'-k',tke(2:end),...
        '_met',tmfn(2:end),'-b',tbfn(2:end),'-k',tkfn(2:end),...
        '_cmax',tcfn,'-b',tbcmx(2:end),'-k',tkc(2:end),'_J',tj(2:end),...
