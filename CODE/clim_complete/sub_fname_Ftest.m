%%%% File naming system
function [fname,simname] = sub_fname_Ftest(param)
...
spath = '/Volumes/FEISTY/NC/Matlab_new_size/';

if (~isfolder(['/Volumes/FEISTY/NC/Matlab_new_size/',simname,'/Climatology']))
    mkdir(['/Volumes/FEISTY/NC/Matlab_new_size/',simname,'/Climatology'])
end

if (isempty(sel))
    fname = [spath,simname, '/Climatology/Climatol_pristine'];
elseif (param.Jsel ~= 0.1)
    fname = [spath,simname, '/Climatology/Climatol_', sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
else
    fname = [spath,simname, '/Climatology/Climatol_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
end
