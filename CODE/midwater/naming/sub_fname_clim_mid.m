%%%% File naming system
function [fname,simname] = sub_fname_clim_mid(param)
...
tw = num2str(1000+int64(100 * param.W));
tbe = num2str(100+int64(100 * param.bent_eff));
% ADD! Proportion of zooplankton that vertically migrate: param.zmigr
...
simname = [coup,'_enc',tefn,'-b',tbenc(2:end),'_m',tmfn,'-b',tbfn(2:end),'-k',tkfn(2:end),'_c',tcfn,'-b',tbcmx(2:end),'_D',td(2:end),'_A',ta(2:end),'_Sm',tsm(2:end),'_Migr',tw(2:end),'_nmort',tmort,'_BE',tbe(2:end),'_noCC_RE',tre(2:end)];

%! Setup netcdf path to store to
spath = '/Volumes/MIP/NC/Midwater/';
...
if (isempty(sel))
    fname = [spath,simname, '/Climatol_midwater_pristine'];
elseif (param.Jsel ~= 0.1)
    fname = [spath,simname, '/Climatol_midwater_', sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
elseif (param.MFsel ~= param.LPsel)
    fname = [spath,simname, '/Climatol_midwater_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
else
    fname = [spath,simname, '/Climatol_midwater_', sel,'_fish',tfish(2:end)];
end
