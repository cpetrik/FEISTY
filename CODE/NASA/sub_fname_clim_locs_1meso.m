%%%% File naming system
function [fname,simname] = sub_fname_clim_locs_1meso(param)
...
if (frate==0)
    fname = ['/Volumes/FEISTY/NC/Matlab_new_size/',simname, '/Climatol_locs_1meso_pristine'];
elseif (param.Jsel~=0.1)
    fname = ['/Volumes/FEISTY/NC/Matlab_new_size/',simname, '/Climatol_locs_1meso_', sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
elseif (param.MFsel~=param.LPsel)
    fname = ['/Volumes/FEISTY/NC/Matlab_new_size/',simname, '/Climatol_locs_1meso_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
else
    fname  = ['/Volumes/FEISTY/NC/Matlab_new_size/',simname, '/Climatol_locs_1meso_', sel,'_fish',tfish(2:end)];
end
