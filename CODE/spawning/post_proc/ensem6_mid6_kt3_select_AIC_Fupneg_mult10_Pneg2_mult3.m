...
mis_nnu = mis(:,2:4);
nid = find(mis_nnu(:,1) < 0);
unid = intersect(kid,nid);
mis_nnu(unid,1) = mis_nnu(unid,1) .* 10;

pid = find(mis_nnu(:,2) < (-1*log10(2)));
mis_nnu(pid,2) = mis_nnu(pid,2) .* 3;

mis_nnu = reshape(mis_nnu,45*3,1);
...
LLnnu = -n/2 * log(2*pi*sig) - (1/(2*sig)) * sum(mis_nnu.^2);
LL_all = [LL0;LL];
