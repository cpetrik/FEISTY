...
for W = 1%:NWID
    ...
    for YR = 1:YEARS % years
        ...
        D_Med_d = zeros(NZID,DAYS);
        D_Lrg_p = zeros(NZID,DAYS);
        D_Lrg_d = zeros(NZID,DAYS);
        D_Bent_bio = zeros(NZID,DAYS);
        ...
        %     netcdf.putVar(ncidB,vidbioB,[0 MNT-1],[NX 1],mean(D_Bent_bio(:,a(i):b(i)),2));
        %     netcdf.putVar(ncidB,vidTB,MNT-1,1,MNT);
        %
        %     netcdf.putVar(ncidSF,vidbioSF,[0 MNT-1],[NX 1],mean(D_Sml_f(:,a(i):b(i)),2));
        %     netcdf.putVar(ncidSP,vidbioSP,[0 MNT-1],[NX 1],mean(D_Sml_p(:,a(i):b(i)),2));
        %     netcdf.putVar(ncidSD,vidbioSD,[0 MNT-1],[NX 1],mean(D_Sml_d(:,a(i):b(i)),2));
        %     netcdf.putVar(ncidMF,vidbioMF,[0 MNT-1],[NX 1],mean(D_Med_f(:,a(i):b(i)),2));
        %     netcdf.putVar(ncidMP,vidbioMP,[0 MNT-1],[NX 1],mean(D_Med_p(:,a(i):b(i)),2));
        %     netcdf.putVar(ncidMD,vidbioMD,[0 MNT-1],[NX 1],mean(D_Med_d(:,a(i):b(i)),2));
        %     netcdf.putVar(ncidLP,vidbioLP,[0 MNT-1],[NX 1],mean(D_Lrg_p(:,a(i):b(i)),2));
        %     netcdf.putVar(ncidLD,vidbioLD,[0 MNT-1],[NX 1],mean(D_Lrg_d(:,a(i):b(i)),2));
        %
        % end %Monthly means
        ...
        %3-D
        netcdf.putVar(ncidSF,vidbioSF,[WID(W)-1,0,YR-1],[1,NZID,1],mean(D_Sml_f,2));
        netcdf.putVar(ncidSP,vidbioSP,[WID(W)-1,0,YR-1],[1,NZID,1],mean(D_Sml_p,2));
        netcdf.putVar(ncidSD,vidbioSD,[WID(W)-1,0,YR-1],[1,NZID,1],mean(D_Sml_d,2));
        netcdf.putVar(ncidMF,vidbioMF,[WID(W)-1,0,YR-1],[1,NZID,1],mean(D_Med_f,2));
        netcdf.putVar(ncidMP,vidbioMP,[WID(W)-1,0,YR-1],[1,NZID,1],mean(D_Med_p,2));
        netcdf.putVar(ncidLP,vidbioLP,[WID(W)-1,0,YR-1],[1,NZID,1],mean(D_Lrg_p,2));
        %2-D
        netcdf.putVar(ncidMD,vidbioMD,[WID(W)-1,YR-1],[1,1],mean(D_Med_d,2));
        netcdf.putVar(ncidLD,vidbioLD,[WID(W)-1,YR-1],[1,1],mean(D_Lrg_d,2));
        netcdf.putVar(ncidB,vidbioB,[WID(W)-1,YR-1],[1,1],mean(D_Bent_bio,2));
