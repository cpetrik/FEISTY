...
for W = 1%:NWID
    ...
    for YR = 1:YEARS % years
        ...
        if (rem(YR,5)==0) % every 5 yrs
            yrtick = yrtick +1;
            %3-D
            netcdf.putVar(ncidSF,vidbioSF,[WID(W)-1,0,yrtick-1],[1,NZID,1],mean(D_Sml_f,2));
            netcdf.putVar(ncidSP,vidbioSP,[WID(W)-1,0,yrtick-1],[1,NZID,1],mean(D_Sml_p,2));
            netcdf.putVar(ncidSD,vidbioSD,[WID(W)-1,0,yrtick-1],[1,NZID,1],mean(D_Sml_d,2));
            netcdf.putVar(ncidMF,vidbioMF,[WID(W)-1,0,yrtick-1],[1,NZID,1],mean(D_Med_f,2));
            netcdf.putVar(ncidMP,vidbioMP,[WID(W)-1,0,yrtick-1],[1,NZID,1],mean(D_Med_p,2));
            netcdf.putVar(ncidLP,vidbioLP,[WID(W)-1,0,yrtick-1],[1,NZID,1],mean(D_Lrg_p,2));
            %2-D
            netcdf.putVar(ncidMD,vidbioMD,[WID(W)-1,yrtick-1],[1,1],mean(D_Med_d,2));
            netcdf.putVar(ncidLD,vidbioLD,[WID(W)-1,yrtick-1],[1,1],mean(D_Lrg_d,2));
            netcdf.putVar(ncidB,vidbioB,[WID(W)-1,yrtick-1],[1,1],mean(D_Bent_bio,2));
            netcdf.putVar(ncidB,vidTB,yrtick-1,1,yrtick);
        end % every 5 yrs
