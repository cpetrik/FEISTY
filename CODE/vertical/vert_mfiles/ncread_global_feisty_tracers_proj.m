...
%%
%ncdisp([fpath 'ocean_cobalt_feisty_tracers_z.199001-199412.Sf_B.nc'])
...
for y=1:2%:6
    ...
    %% SF
    ncid = netcdf.open([fpath 'ocean_cobalt_feisty_tracers_z.',...
    ...
    %% SP
    ncid = netcdf.open([fpath 'ocean_cobalt_feisty_tracers_2d.',...
    ...
    tMd = mean(Md_B,1,'omitnan');
    tMd = squeeze(mean(tMd,2,'omitnan'));

    tLd = mean(Ld_B,1,'omitnan');
    tLd = squeeze(mean(tLd,2,'omitnan'));

    tBe = mean(BE_B,1,'omitnan');
    ...
    sMd = mean(Md_B,3,'omitnan');
    sLd = mean(Ld_B,3,'omitnan');
    sBe = mean(BE_B,3,'omitnan');
...
save([fpath 'ocean_cobalt_feisty_tracers_z.199001',...
        '-',num2str(en(y)),'12_means.nc'],'tSF','tSP','tSD',...
