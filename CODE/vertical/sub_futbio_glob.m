...
function [Sf,Sp,Sd,Mf,Mp,Md,Lp,Ld,BENT,ENVR] = sub_futbio_glob(DY,ESM,Sf,Sp,Sd,Mf,Mp,Md,Lp,Ld,BENT,param)

dfrate = param.dfrate;

%%% ESM information
ENVR = get_gESM(ESM,param,DY);
...
% Pelagic-demersal coupling = depth levels occupied
if (param.pdc == 0)
...
Md.met = sub_met(ENVR.Tb,Md.td,param.M_m,param);
Lp.met = sub_met(ENVR.Tp,Lp.td,param.M_l,param);
Ld.met = sub_met(ENVR.Tdem,Ld.td,param.M_l,param);
...
Md.enc_be = sub_enc(param,ENVR.Tb,param.M_m,BENT.mass,Md.td,param.MD_phi_BE/100);
...
Ld.enc_f  = sub_enc(param,ENVR.Tdem,param.M_l,Mf.bio,Ld.td,param.LD_phi_MF);
Ld.enc_p  = sub_enc(param,ENVR.Tdem,param.M_l,Mp.bio,Ld.td,param.LD_phi_MP);
Ld.enc_d  = sub_enc(param,ENVR.Tdem,param.M_l,Md.bio,Ld.td,param.LD_phi_MD/100);
Ld.enc_be = sub_enc(param,ENVR.Tdem,param.M_l,BENT.mass,Ld.td,param.LD_phi_BE/100);
...
Md.con_be = sub_cons(param,ENVR.Tb,Md.td,param.M_m,Md.enc_be);
...
Ld.con_f  = sub_cons(param,ENVR.Tdem,Ld.td,param.M_l,[Ld.enc_f,Ld.enc_p,Ld.enc_d,Ld.enc_be]);
Ld.con_p  = sub_cons(param,ENVR.Tdem,Ld.td,param.M_l,[Ld.enc_p,Ld.enc_f,Ld.enc_d,Ld.enc_be]);
Ld.con_d  = sub_cons(param,ENVR.Tdem,Ld.td,param.M_l,[Ld.enc_d,Ld.enc_p,Ld.enc_f,Ld.enc_be]);
Ld.con_be = sub_cons(param,ENVR.Tdem,Ld.td,param.M_l,[Ld.enc_be,Ld.enc_f,Ld.enc_p,Ld.enc_d]);
...
% Total consumption rates (could factor in handling times here; g m-3 d-1)
Sf.I = Sf.con_zm;
Sp.I = Sp.con_zm;
Sd.I = Sd.con_zm;
...
Md.clev = sub_clev(param,Md.I,ENVR.Tb,Md.td,param.M_m);
Lp.clev = sub_clev(param,Lp.I,ENVR.Tp,Lp.td,param.M_l);
Ld.clev = sub_clev(param,Ld.I,ENVR.Tdem,Ld.td,param.M_l);

% Death rates (g m-3 d-1)
...
Md.nmort = sub_nmort(param,ENVR.Tb,Md.td,param.M_m);
Lp.nmort = sub_nmort(param,ENVR.Tp,Lp.td,param.M_l);
Ld.nmort = sub_nmort(param,ENVR.Tdem,Ld.td,param.M_l);
