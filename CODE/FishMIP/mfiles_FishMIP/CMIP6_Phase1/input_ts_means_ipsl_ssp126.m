...
load([fpath 'ipsl_ssp126_temp_100_monthly_2015_2100.mat'],'temp_100');
load([fpath 'ipsl_ssp126_temp_btm_monthly_2015_2100.mat'],'temp_btm');
load([fpath 'ipsl_ssp126_zmeso_100_monthly_2015_2100.mat'],'zmeso_100');
load([fpath 'ipsl_ssp126_det_btm_monthly_2015_2100.mat']); %,'det_btm'
...
en=12:12:nt;
for n=1:length(st)
    Tp_1yr_ssp126(:,n)=nanmean(Tp(WID,st(n):en(n)),2);
    Tb_1yr_ssp126(:,n)=nanmean(Tb(WID,st(n):en(n)),2);
    det_1yr_ssp126(:,n)=nanmean(det(WID,st(n):en(n)),2);
    mz_1yr_ssp126(:,n)=nanmean(mzp(WID,st(n):en(n)),2);
end

%% save
s2time=time;
s2yrs=yrs;
save([fpath 'ipsl_ssp126_inputs_ts_nanmeans.mat'],'s2time','s2yrs',...
    'Tp_1yr_ssp126','Tb_1yr_ssp126','det_1yr_ssp126','mz_1yr_ssp126');
