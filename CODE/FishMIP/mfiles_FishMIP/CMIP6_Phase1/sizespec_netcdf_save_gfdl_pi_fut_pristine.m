...
load('/Volumes/MIP/Fish-MIP/CMIP6/GFDL/preindust/gfdl_pi_temp_btm_monthly_1950_2100.mat',...
    'yr','time');
t_all = time;
...
%% Just future years
allSpel = allSpel(:,runs2);
allMpel = allMpel(:,runs2);
allLpel = allLpel(:,runs2);
allSdem = allSdem(:,runs2);
allMdem = allMdem(:,runs2);
allLdem = allLdem(:,runs2);
...
fname1 = 'feisty_gfdl-esm4_nobasd_picontrol_nat_default_';
...
netcdf.putVar(ncidSB,vidtSB,ftime);
...
netcdf.putVar(ncidSD,vidtSD,ftime);
...
netcdf.putVar(ncidCB,vidtCB,ftime);
...
netcdf.putVar(ncid30,vidt30,ftime);
...
netcdf.putVar(ncid90,vidt90,ftime);
