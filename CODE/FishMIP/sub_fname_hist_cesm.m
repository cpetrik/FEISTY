%%%% File naming system
function [fname,simname] = sub_fname_hist_cesm(frate)
...
if (~isdir(['/Volumes/GFDL/NC/FishMIP/CESM1-BEC/',simname]))
    mkdir(['/Volumes/GFDL/NC/FishMIP/CESM1-BEC/',simname])
end

%! Setup netcdf path to store to
if (frate==0)
    fname = ['/Volumes/GFDL/NC/FishMIP/CESM1-BEC/',simname, '/Historic_pristine'];
elseif (Jsel~=0.1)
    fname = ['/Volumes/GFDL/NC/FishMIP/CESM1-BEC/',simname, '/Historic_', sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
elseif (MFsel~=LPsel)
    fname = ['/Volumes/GFDL/NC/FishMIP/CESM1-BEC/',simname, '/Historic_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
else
    fname  = ['/Volumes/GFDL/NC/FishMIP/CESM1-BEC/',simname, '/Historic_', sel,'_fish',tfish(2:end)];
end
