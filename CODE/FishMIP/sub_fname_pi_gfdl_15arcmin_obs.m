%%%% File naming system
function [fname,simname,outdir] = sub_fname_pi_gfdl_15arcmin_obs(param)
...
if (frate==0)
    fname = [outdir, 'PI_obsclim_pristine'];
elseif (param.Jsel~=0.1)
    fname = [outdir, 'PI_obsclim_', sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
elseif (param.MFsel~=param.LPsel)
    fname = [outdir, 'PI_obsclim_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
else
    fname = [outdir, 'PI_obsclim_', sel,'_fish',tfish(2:end)];
end
