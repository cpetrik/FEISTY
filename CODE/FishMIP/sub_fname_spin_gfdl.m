...
if (~isfolder(['/Volumes/MIP/NC/FishMIP/GFDL_CMIP6/',simname]))
    mkdir(['/Volumes/MIP/NC/FishMIP/GFDL_CMIP6/',simname])
end

%! Setup netcdf path to store to
if (frate==0)
    fname = ['/Volumes/MIP/NC/FishMIP/GFDL_CMIP6/',simname, '/Spinup_Pre'];
elseif (param.Jsel~=0.1)
    fname = ['/Volumes/MIP/NC/FishMIP/GFDL_CMIP6/',simname, '/Spinup_Pre_', sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
elseif (param.MFsel~=param.LPsel)
    fname = ['/Volumes/MIP/NC/FishMIP/GFDL_CMIP6/',simname, '/Spinup_Pre_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
else
    fname  = ['/Volumes/MIP/NC/FishMIP/GFDL_CMIP6/',simname, '/Spinup_Pre_', sel,'_fish',tfish(2:end)];  
end
