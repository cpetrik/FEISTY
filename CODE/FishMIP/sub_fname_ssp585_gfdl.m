%%%% File naming system
function [fname,simname] = sub_fname_ssp585_gfdl(param)
...
if (~isfolder(['/Volumes/petrik-lab/Feisty/NC/FishMIP/GFDL_CMIP6/',simname]))
    mkdir(['/Volumes/petrik-lab/Feisty/NC/FishMIP/GFDL_CMIP6/',simname])
end

%! Setup netcdf path to store to
if (frate==0)
    fname = ['/Volumes/petrik-lab/Feisty/NC/FishMIP/GFDL_CMIP6/',simname, '/SSP585'];
elseif (param.Jsel~=0.1)
    fname = ['/Volumes/petrik-lab/Feisty/NC/FishMIP/GFDL_CMIP6/',simname, '/SSP585_', sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
elseif (param.MFsel~=param.LPsel)
    fname = ['/Volumes/petrik-lab/Feisty/NC/FishMIP/GFDL_CMIP6/',simname, '/SSP585_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
else
    fname  = ['/Volumes/petrik-lab/Feisty/NC/FishMIP/GFDL_CMIP6/',simname, '/SSP585_', sel,'_fish',tfish(2:end)];  
end
