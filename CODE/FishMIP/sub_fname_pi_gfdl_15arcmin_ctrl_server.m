%%%% File naming system
function [fname,simname,outdir] = sub_fname_pi_gfdl_15arcmin_ctrl_server(param,vers)
...
if (frate==0)
    fname = [outdir, 'PI_ctrlclim_pristine'];
elseif (param.Jsel~=0.1)
    fname = [outdir, 'PI_ctrlclim_', sel,'_fish',tfish(2:end),'_Juve',tJ(2:end),'_',vers];
elseif (param.MFsel~=param.LPsel)
    fname = [outdir, 'PI_ctrlclim_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end),'_',vers];
else
    fname = [outdir, 'PI_ctrlclim_', sel,'_fish',tfish,'_',vers];
end
